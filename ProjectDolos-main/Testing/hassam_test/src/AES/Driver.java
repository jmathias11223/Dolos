...
public class Driver {
	...
	public static String run(AESInput input) {
		if(input.encryptOrDecrypt().equals("Encrypt")) {
			return encrypt(textToHex(input.getText()), keyExpansion(input.getKey(), AESType.AES256), AESType.AES256, true); // AES256, CBC mode
		}
		return hexToText(decrypt(input.getText(), keyExpansion(input.getKey(), AESType.AES256), AESType.AES256, true));
	}
	...
	public static String invMixColumns(String[][] text) {
		...
		for(int col = 0; col < text[0].length; col++) { // Traverse each column of text
			for(int row = 0; row < text.length; row++) { // Traverse each column entry in text
				long[] add = new long[4];
				for(int matRow = 0; matRow < matrix.length; matRow++) {
					int curr = (int)Long.parseLong(text[matRow][col], 16);
					int[] addArr = MatrixMultiplication.multiplication(hexToBinary(curr), hexToBinary(matrix[row][matRow]));
					
					int convert = 0;
					// Converts binary array to an integer
					for(int i = addArr.length - 1; i >= 0; i--) {
						if(addArr[i] == 1) {
							convert += Math.pow(2,  addArr.length - 1 - i);
						}
					}
					add[matRow] = convert;
					
					//add[matRow] = curr * matrix[row][matRow];
				}
				long sum = add[0] ^ add[1] ^ add[2] ^ add[3];
				...
				int pos = 0;
				while(pos < 16 && bin[pos] == 0) {
					pos++;
				}
				int len = 15 - pos;
				while(len > 8) {	// Check for overflow
					sum = sum ^ (AES_POLYNOMIAL << len - 9);
					len--;
				}
